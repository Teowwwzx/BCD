...
contract DSCMMarketplace is ReentrancyGuard, Ownable, Pausable, AccessControl {
    ...
    function createListing(
        ...
        require(
            _price >= MIN_LISTING_PRICE && _price <= MAX_LISTING_PRICE,
    ...
    function assignTransporter(
        ...
        require(
            ...
            "Only seller can assign transporter"
        );
        require(
    ...
    function processRefund(uint256 _orderId) external onlyRole(ADMIN_ROLE) {
        Order storage order = orders[_orderId];
        ...
        require(
            order.status != OrderStatus.Completed &&
                order.status != OrderStatus.Cancelled,
            "Cannot refund a completed or cancelled order"
        );
        require(order.escrowAmount > 0, "No funds in escrow to refund");

        uint256 refundAmount = order.escrowAmount;
        order.escrowAmount = 0; // Clear the escrow amount
        order.status = OrderStatus.Cancelled; // Use 'Cancelled' as the final state on-chain
